...
export function SpawnPanel({ worldRef, agents, weapons, isPaused, hasApiKey, onPauseToggle }: Props) {
  ...
  // Per-agent live-edit state (maxHP / weaponId / model).
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 flex flex-col gap-4">
          ...
          {!hasApiKey && (
            <div className="rounded-md bg-amber-50 border border-amber-200 px-3 py-2 text-[11px] text-amber-700 leading-snug">
              No API key set. Open Settings (gear icon) to configure OpenRouter.
            </div>
          )}
          ...
          <div className="flex flex-col gap-2">
            ...
            <input
              ...
              placeholder="Name"
              maxLength={32}
              className="px-3 py-1.5 text-xs border border-gray-200 rounded-md focus:outline-none focus:border-gray-400"
            />
            ...
            <textarea
              ...
              rows={2}
              className="px-3 py-1.5 text-xs border border-gray-200 rounded-md focus:outline-none focus:border-gray-400 resize-none"
            />
            ...
            <input
              ...
              placeholder="Model (e.g. google/gemini-flash-1.5)"
              className="px-3 py-1.5 text-xs border border-gray-200 rounded-md focus:outline-none focus:border-gray-400"
            />
            ...
            <button
              onClick={handleSpawn}
              disabled={!name.trim()}
              className="py-1.5 text-xs font-semibold rounded-md bg-gray-900 text-white hover:bg-gray-700 transition-colors disabled:opacity-40"
            >
              Spawn
            </button>
          </div>
                ...
                return (
                  <div key={a.id} className="flex flex-col rounded-lg border border-gray-100 overflow-hidden">
                    ...
                    <div
                      ...
                      onClick={() => setInspecting(v => v === a.id ? null : a.id)}
                    >
                      ...
                      <div className="w-12 h-1.5 bg-gray-100 rounded-full overflow-hidden shrink-0">
                        {(() => { const f = a.health.maxHP > 0 ? Math.max(0, a.health.currentHP / a.health.maxHP) : 0; return (
                        <div
                          className="h-full rounded-full transition-all"
                          style={{
                            width: `${f * 100}%`,
                            background: f > 0.5 ? '#4caf50' : f > 0.25 ? '#ff9800' : '#f44336',
                          }}
                        />); })()}
                      </div>
                      ...
                      <span className={`
                        ...
                        ${a.lifecycleState === 'thinking'
                          ? 'bg-blue-50 border-blue-200 text-blue-600'
                          : a.lifecycleState === 'acting'
                      ...
                      <button
                        ...
                        className="text-gray-300 hover:text-red-400 transition-colors text-sm leading-none ml-1"
                      >✕</button>
                    </div>
                    ...
                    {inspecting === a.id && (
                      <div className="px-3 pb-3 pt-2 border-t border-gray-100 bg-gray-50 flex flex-col gap-3">
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          <div className="flex gap-1">
                            <input
                              type="text"
                              value={modVal}
                              onChange={e => setEditModel(p => ({ ...p, [a.id]: e.target.value }))}
                              placeholder="Model"
                              className="flex-1 px-2 py-1 text-xs border border-gray-200 rounded focus:outline-none focus:border-gray-400 bg-white"
                            />
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <input
                              ...
                              min={1}
                              value={hpVal}
                              onChange={e => setEditHP(p => ({ ...p, [a.id]: Math.max(1, parseInt(e.target.value) || 1) }))}
                              className="w-16 px-2 py-1 text-xs border border-gray-200 rounded focus:outline-none focus:border-gray-400 bg-white text-right"
                            />
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <select
                              value={wepVal}
                              onChange={e => applyAgentWeapon(a.id, e.target.value)}
                              className="flex-1 px-2 py-1 text-xs border border-gray-200 rounded focus:outline-none focus:border-gray-400 bg-white"
                            >
                              <option value="">None</option>
                        ...
                        <div className="flex flex-col gap-1">
                          <span className="text-[10px] uppercase tracking-widest text-gray-400">Memory</span>
                          <div className="flex flex-col gap-0.5 max-h-28 overflow-y-auto">
                            {a.memory.length === 0
                              ? <span className="text-[11px] text-gray-400">Empty</span>
                              : [...a.memory].reverse().map((m, i) => (
                                  <span key={i} className="text-[11px] text-gray-600 leading-snug">{m}</span>
                                ))
                            }
                          </div>
                        </div>
          ...
          {agents.length === 0 && (
            <p className="text-[11px] text-gray-400 text-center pt-2">
              No agents yet. Fill in the form above and click Spawn.
            </p>
          )}
        </div>
